---
page_id: 'source:environment-prod-or-local-a93beb0c'
kind: source
cssclasses:
  - swarmvault
  - sv-source
title: Environment (prod or local)
source_class: first_party
tags:
  - source
source_ids:
  - environment-prod-or-local-a93beb0c
project_ids: []
node_ids:
  - 'source:environment-prod-or-local-a93beb0c'
  - 'concept:true'
  - 'concept:false'
  - 'concept:debug'
  - 'concept:tokenizer'
  - 'concept:cache'
  - 'concept:cycle'
  - 'entity:debug'
  - 'entity:verbosity'
  - 'entity:set'
  - 'entity:monolog'
  - 'entity:use'
  - 'entity:roadrunner'
freshness: fresh
status: active
confidence: 1
created_at: '2026-05-03T05:11:46.022Z'
updated_at: '2026-05-03T05:32:23.223Z'
compiled_from:
  - environment-prod-or-local-a93beb0c
managed_by: system
backlinks:
  - 'concept:true'
  - 'concept:false'
  - 'concept:debug'
  - 'concept:tokenizer'
  - 'concept:cache'
  - 'concept:cycle'
  - 'entity:debug'
  - 'entity:verbosity'
  - 'entity:set'
  - 'entity:monolog'
  - 'entity:use'
  - 'entity:roadrunner'
  - 'output:source-briefs/directory-op-597d97ce'
schema_hash: 874431dbbfdec0b254a4aa1bf002900574c9b485735a883690c2becb5f717720
source_hashes:
  environment-prod-or-local-a93beb0c: a93beb0cbb31ac8ce316e5326fcd21a6b6b52911cea462777294ef0c6c8c7ffd
source_semantic_hashes:
  environment-prod-or-local-a93beb0c: a93beb0cbb31ac8ce316e5326fcd21a6b6b52911cea462777294ef0c6c8c7ffd
---
# Environment (prod or local)

Source ID: `environment-prod-or-local-a93beb0c`
Source Kind: `text`
Source Path: `/Users/rk/GolandProjects/op/poc/spiral-emit/.env.sample`

Source Class: `first_party`


## Summary

Environment (prod or local) APP_ENV=local # Debug mode set to TRUE disables view caching and enables higher verbosity DEBUG=true # Verbosity level VERBOSITY_LEVEL=verbose # basic, verbose, debug # Set to an application specific value, used to encrypt/decrypt cookies etc ENCRYPTER_KEY={encrypt-key} # Monolog MONOLOG_DEFAULT_CHANNEL=default # Use "roadrunner" channel if you want to use RoadRunner logger MONOLOG_DEFAULT_LEVEL=DEBUG # DEBUG, INFO, NOTICE, WARNING, ERROR, CRITICAL, ALERT, EMERGENCY # Telemetry TELEMETRY_DRIVER=null # Tokenizer TOKENIZER_CACHE_TARGETS=false TOKENIZER_LOAD_CLASSES=true TOKENIZER_LOAD_ENUMS=true TOKENIZER_LOAD_INTERFACES=true # View component options VIEW_CACHE=false # Session SESSION_LIFETIME=86400 SESSION_COOKIE=sid # Authorization AUTH_TOKEN_TRANSPORT=cookie AUTH_TOKEN_STORAGE=session # Set to TRUE to disable confirmation in migrate commands SAFE_MIGRATIONS=true # Database connection options DB_CONNECTION=sqlite DB_LOG_QUERY_PARAMETERS=false DB_LOG_INTERPOLATED_QUERIES=false DB_WITH_DATETIME_MICROSECONDS=false DB_DATABASE=spiral DB_HOST=127.0.0.1 DB_PORT=3307 DB_USERNAME=root DB_PASSWORD=password # Cycle Bridge (Don't forget to set CYCLE_SCHEMA_CACHE to true in production) CYCLE_SCHEMA_CACHE=false CYCLE_SCHEMA_WARMUP=false # Internationalization LOCALE=en

## Concepts

- [[concepts/true|true]]: Frequently referenced concept in Environment (prod or local).
- [[concepts/false|false]]: Frequently referenced concept in Environment (prod or local).
- [[concepts/debug|debug]]: Frequently referenced concept in Environment (prod or local).
- [[concepts/tokenizer|tokenizer]]: Frequently referenced concept in Environment (prod or local).
- [[concepts/cache|cache]]: Frequently referenced concept in Environment (prod or local).
- [[concepts/cycle|cycle]]: Frequently referenced concept in Environment (prod or local).

## Entities

- [[entities/debug|Debug]]: Named entity mentioned in Environment (prod or local).
- [[entities/verbosity|Verbosity]]: Named entity mentioned in Environment (prod or local).
- [[entities/set|Set]]: Named entity mentioned in Environment (prod or local).
- [[entities/monolog|Monolog]]: Named entity mentioned in Environment (prod or local).
- [[entities/use|Use]]: Named entity mentioned in Environment (prod or local).
- [[entities/roadrunner|RoadRunner]]: Named entity mentioned in Environment (prod or local).

## Claims

- Environment (prod or local) APP_ENV=local # Debug mode set to TRUE disables view caching and enables higher verbosity DEBUG=true # Verbosity level VERBOSITY_LEVEL=verbose # basic, verbose, debug # Set to an application specific value, used to encrypt/decrypt cookies etc ENCRYPTER_KEY={encrypt-key} # Monolog MONOLOG_DEFAULT_CHANNEL=default # Use "roadrunner" channel if you want to use RoadRunner logger MONOLOG_DEFAULT_LEVEL=DEBUG # DEBUG, INFO, NOTICE, WARNING, ERROR, CRITICAL, ALERT, EMERGENCY # Telemetry TELEMETRY_DRIVER=null # Tokenizer TOKENIZER_CACHE_TARGETS=false TOKENIZER_LOAD_CLASSES=true TOKENIZER_LOAD_ENUMS=true TOKENIZER_LOAD_INTERFACES=true # View component options VIEW_CACHE=false # Session SESSION_LIFETIME=86400 SESSION_COOKIE=sid # Authorization AUTH_TOKEN_TRANSPORT=cookie AUTH_TOKEN_STORAGE=session # Set to TRUE to disable confirmation in migrate commands SAFE_MIGRATIONS=true # Database connection options DB_CONNECTION=sqlite DB_LOG_QUERY_PARAMETERS=false DB_LOG_INTERPOLATED_QUERIES=false DB_WITH_DATETIME_MICROSECONDS=false DB_DATABASE=spiral DB_HOST=127.0.0.1 DB_PORT=3307 DB_USERNAME=root DB_PASSWORD=password # Cycle Bridge (Don't forget to set CYCLE_SCHEMA_CACHE to true in production) CYCLE_SCHEMA_CACHE=false CYCLE_SCHEMA_WARMUP=false # Internationalization LOCALE=en [source:environment-prod-or-local-a93beb0c]

## Questions

- How does true relate to Environment (prod or local)?
- How does false relate to Environment (prod or local)?
- How does debug relate to Environment (prod or local)?

## Related Outputs

- [[outputs/source-briefs/directory-op-597d97ce|Source Brief: op]]

